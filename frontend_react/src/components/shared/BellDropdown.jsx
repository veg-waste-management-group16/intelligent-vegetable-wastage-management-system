...
import { NOTIF_API, fmtDate } from '../../api';
...
export function BellDropdown({ role }) {
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      {open && (
        <div style={{
          position: 'fixed', top: 78, right: 68,
          width: 330, background: 'white', borderRadius: 16,
              ...
              : notifs.map(n => {
                ...
                return (
                  <div key={n.id} className={`bell-dd-item${isUnread ? ' unread' : ''}`}>
                    ...
                    <div className="bell-dd-msg">{n.message || n.notification_message}</div>
                    <div className="bell-dd-time">🕐 {fmtDate(n.sentAt || n.sent_at)} · {n.sentByName || n.sent_by_name || 'Admin'}</div>
                  </div>
                );
              })
            }
